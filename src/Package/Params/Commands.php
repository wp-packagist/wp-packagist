...
class Commands
{
    ...
    public function sanitize_commands($array, $validate = false)
    {
        ...
        //Check is String
        if (is_string($array)) {
            $valid->add_error(Package::_e('package', 'is_string', array("[key]" => "commands: { ..")));
        } elseif (empty($array)) {
            //Check Empty Array
            $valid->add_error(Package::_e('package', 'empty_val', array("[key]" => "commands: { ..")));
        } else {
            //Check is Assoc array
            if (\WP_CLI_Util::is_assoc_array($array) === false) {
                $valid->add_error(Package::_e('package', 'er_valid', array("[key]" => "commands: { ..")));
            } else {
                ...
                foreach ($array as $command => $type) {
                    //Check Empty
                    if (empty($type)) {
                        $valid->add_error(Package::_e('package', 'empty_val', array("[key]" => "commands: { '" . $command . "' ..")));
                        break;
                    }
                    ...
                    if (is_array($type)) {
                        ...
                        if (\WP_CLI_Util::is_assoc_array($type) === true) {
                            $valid->add_error(Package::_e('package', 'er_valid', array("[key]" => "commands: { '" . $command . "' ..")));
                            break;
                        } else {
                            //Push to List
                ...
                if ( ! $valid->is_cli_error()) {
                    ...
                    foreach ($command_list as $commends) {
                        ...
                        //Check if array
                        if (is_array($type)) {
                            $valid->add_error(Package::_e('package', 'er_string_command', array("[key]" => $command)));
                            break;
                        } elseif (empty(\WP_CLI_Util::to_lower_string($type))) {
                            //Check is empty path
                            $valid->add_error(Package::_e('package', 'empty_val', array("[key]" => "commands: { '" . $command . "' ..")));
                            break;
                        } else {
                            ...
                            if ( ! in_array($type, $this->package_config['wp-cli-command'])) {
                                ...
                                if (':' === substr($command, 1, 1)) {
                                    $valid->add_error(Package::_e('package', 'er_contain_drive_cmd', array("[key]" => $command)));
                                    break;
                                } else {
                                    //Sanitize folder Path
                                    $type = ltrim(\WP_CLI_FileSystem::normalize_path($type), "/");
