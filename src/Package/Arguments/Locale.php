...
class Locale {
	...
	public static function fetch_wordpress_locale() {
		...
		$list = CLI::http_request( Package::get_config( 'wordpress_api', 'translations' ) );
		if ( $list != false ) {
	...
	public static function get_wordpress_locale( $key = false, $force_update = false ) {
		...
		if ( $force_update === false ) {
			...
			if ( isset( $json_data ) and FileSystem::check_file_age( $file_path, Package::get_config( 'package', 'locale', 'age' ) ) === false ) {
				$list = $json_data;
			}
		}
